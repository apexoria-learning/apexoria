...
### 2026-08-02 — Google Tag Manager container `GTM-PR4M68SJ` removed from `public/index.html` (perf: GTmetrix Phase 2)
- Category: product-decision
- Fact: Both the GTM head-side `<script>` loader and the `<noscript>` iframe were removed from [public/index.html](../public/index.html) as part of GTmetrix Phase 2 (2026-08-02). The `preconnect` to `https://www.googletagmanager.com` was also removed (only GA4's own `gtag.js` uses that host now — its `<script async src="…/gtag/js?id=G-…">` provides its own connection). Container `GTM-PR4M68SJ` was empty (no active tags — LinkedIn Insight / Meta Pixel had not been wired yet), so nothing broke. Site is now on FOUR analytics stacks: PostHog (inline autocapture), Vercel Analytics (`<Analytics />` in App.js), GA4 (inline gtag per 2026-07-21 entry), Vercel Speed Insights (`<SpeedInsights />` in App.js).
- Affects: Frontend + SEO Auditor + QA + Deployment
- Impact: TBT win of ~80–120 ms + 163 KB off the third-party bundle. GA4 is UNAFFECTED — it has its own inline install (2026-07-21 entry) that does not depend on GTM. `trackEvent()` in [src/lib/analytics.js](../src/lib/analytics.js) still pushes to `window.dataLayer`, so re-adding GTM later is a 15-line HTML restore with zero product-code changes. If you re-add GTM: (a) put the head snippet BACK immediately after the GA4 gtag block, (b) put the `<noscript>` iframe BACK immediately after `<body>`, (c) restore the `preconnect` link, (d) do NOT add a GA4 Configuration tag inside the container UI (per 2026-07-27 entry — that rule still holds and is the reason GTM removal was safe here). The `initAnalytics()` runtime GTM injector in `src/lib/analytics.js` remains as a fallback path — leave it in, do not set `REACT_APP_GTM_ID` in Vercel env unless you deliberately want a runtime-injected container.
- Source: user picked option A in chat on 2026-08-02 during GTmetrix Phase 2 planning ("Full plan - drop one analytics stack" → Orchestrator recommended GTM → user confirmed with "A"). Container-empty status was confirmed implicitly by picking option A (which was gated on Q1=A "container is empty / paused").
- Supersedes: 2026-07-27 — Google Tag Manager installed inline (container `GTM-PR4M68SJ`) — do NOT add a GA4 Config tag inside the container (the "do NOT add GA4 inside the container" rule stays historically relevant if GTM is ever restored, but the install itself is gone).
