...
| Data | Google Form is the sole store. The React lead form POSTs directly to `GOOGLE_FORM_ACTION_URL` from the browser (`mode: 'no-cors'`, `FormData` body). On failure the visible WhatsApp CTA in [LeadForm.jsx](../frontend/src/components/site/LeadForm.jsx) is the only recovery path. See [.github/GOTCHAS.md](GOTCHAS.md) — 2026-07-19 entries. |
...
1. **`data-testid` on every interactive or key informational element.** Pull ids from [frontend/src/constants/testIds/](../frontend/src/constants/testIds/). New ids get added to that registry — never inline.
2. **Preserve the anti-spam trio** — now enforced **client-side** in [LeadForm.jsx](../frontend/src/components/site/LeadForm.jsx) after the 2026-07-19 backend deletion: (a) `company_website` honeypot silently drops the submission, (b) 2000ms time-trap between mount and submit, (c) 12s `localStorage` cooldown between successful submits. All three are bypassable by any bot that skips the JS — do not remove them without user override.
...
9. **Motion contract**: new sections use Framer Motion `whileInView` with `{opacity: 0, y: 40}` → `{opacity: 1, y: 0}` and stagger for children. The Lenis wrapper in [frontend/src/App.js](../frontend/src/App.js) must stay intact.
10. **No new UI library** without user approval — reuse [frontend/src/components/ui/](../frontend/src/components/ui/) shadcn primitives.
