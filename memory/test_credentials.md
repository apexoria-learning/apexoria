...
- Lead form POSTs directly from the browser to a Google Form (`REACT_APP_GF_ACTION_URL` in `.env`, `mode: 'no-cors'`, `FormData` body). The linked Google Sheet is the only store; there is no admin surface.
- Brochure download hits the static asset `/apexoria-brochure.pdf` served from `public/`.
