...
- Uploads: `/api/cms/upload-asset` (same auth) commits binaries to `public/uploads/{folder}/{ts}-{name}` in this repo; site-relative URLs (`/uploads/...`) stored inline in `data.js`. Firebase Storage / GCS is not used. Hard cap 3 MB per file (Vercel Hobby request-body limit).
